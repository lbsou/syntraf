        ...
        for if_name, addrs in interfaces.items():
            for if_name2, stats2 in stats.items():
                ...
                if if_name2 == if_name:
                    if stats2.isup:
                        try:
                            scapy.sendp(scapy.Ether()/scapy.IP(dst=dst_ip) / scapy.UDP(sport=max(lst_udp_port_iperf), dport=dst_port) / scapy.Raw(load="KEEPALIVE"), verbose=False, iface=if_name, count=100, inter=0.001)
                            scapy.sendp(scapy.Ether() / scapy.IP(dst=dst_ip) / scapy.UDP(sport=min(lst_udp_port_iperf), dport=dst_port) / scapy.Raw(load="KEEPALIVE"), verbose=False, iface=if_name, count=100, inter=0.001)
